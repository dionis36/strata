...
class LegacyAnalysisService:
    ...
    def analyze_legacy_environment(self, run_id: int, nodes: list, edges: list) -> dict:
        ...
        # 3. Classify Era (Requirement 1)
        php_era = EraClassifier.classify(nodes, edges, stats)
        
        # 4. Calculate Modernization Scores (Requirement 8)
        scores = ModernizationModel.calculate(stats)
        
        # 5. Hosting Risk Level (Requirement 15)
        hosting_risk = "low"
        ...
        logger.info(f"Legacy metrics saved for run {run_id}: Era={php_era}, Framework={framework}")
