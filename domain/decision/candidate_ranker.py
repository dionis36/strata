...
class CandidateRanker:
    """
    Decides the final feasibility and generates a human-readable "Algorithmic Verdict"
    ...
    def rank(self, unit: ExtractionUnit, impact: ImpactMetrics) -> ExtractionCandidate:
        ...
        if impact.data_isolation_difficulty >= 4 or impact.risk_change >= 0.26:
            ...
            reasoning.append("**Algorithmic Verdict: Refactor First.** While logically related, drawing a network boundary around this cluster would create an unstable interface. You must refactor external dependencies *inside* the monolith before extracting.")
            ...
            reasoning.append("**Algorithmic Verdict: Safe to Extract.** This cleanly bounded context can be safely containerized. It lowers or neutralizes total systemic risk.")
        else:
            recommendation = RecommendationCategory.EXTRACT_WITH_CAUTION
            reasoning.append("**Algorithmic Verdict: Proceed with Caution.** A viable candidate, but architectural vigilance is required due to moderate coupling or shared data layers.")
