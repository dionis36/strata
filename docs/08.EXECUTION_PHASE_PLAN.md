...
# 📊 PHASE 3 — STRUCTURED DETERMINISTIC RISK FRAMEWORK
...
### 1️⃣ Feature Normalization Layer
...
```
Component → Structural Feature Vector
```

---

### 2️⃣ Structural Feature Engineering Layer
...
BaseRisk
+ α(CriticalityIndex)
+ β(InstabilityIndicator)
+ γ(CyclePenalty)
```

Where:

- α, β, γ are explicitly documented coefficients
...
## 🔍 PHASE 4.5 — RISK EXPLAINABILITY ENGINE

### Objective

Transform numerical risk assessments into interpretable, decision-support explanations.

### Includes

- Deterministic, rule-based reasoning engine over structural/behavioral metrics
- Bounded, evidence-backed explanations (max 5 per component)
- Extraction of graph context (dependents, cycle members) and file paths
- `/explain/{run_id}` API endpoint
- Overlay UI (modal) for detailed component insights

### Why This Is Crucial Before Phase 5

You cannot confidently extract components (Phase 5) if you cannot trust or explain _why_ they were flagged as risky. Explainability provides the necessary human validation layer.

### Exit Criteria

- Explanations are deterministic and traceable
- UI overlays without breaking primary risk table
- Tag created:
  `v0.6-explainability`
